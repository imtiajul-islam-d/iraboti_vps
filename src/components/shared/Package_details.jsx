...
export default function Package_details({ setShowModal, room, modalRef }) {
  ...
  return (
    <section
      className="fixed top-0 py-5 left-0 w-full h-screen z-[100] bg-white overflow-y-auto"
      data-lenis-prevent
      tabIndex="-1"
      ref={modalRef}
    >
        ...
        <div className="flex flex-col justify-between sm:flex-row sm:items-end">
          <div className="">
            ...
            <div className="grid grid-cols-4 gap-4 lg:gap-12 pt-5 md:pt-[.625rem] last:w-fit">
              <div className="flex items-center gap-2 sm:gap-[.625rem] uppercase">
                {" "}
        ...
        <div className="details mt-7 md:mt-14 space-y-7 md:space-y-14">
          ...
          <div className="img">
            <div className="grid grid-cols-12 grid-rows-2 gap-[.625rem]">
              <PhotoProvider>
                {room?.images?.map((shot, i) => (
                  <PhotoView key={i} src={shot?.img}>
                    <Image
                      src={shot?.img}
                      alt="room-shot"
                      width={1024}
                      ...
                      className={`${
                        i === 0
                          ? "md:col-span-6 col-span-12 row-span-2"
          ...
          <div className="infos md:w-4/6">
            ...
            <div className="space-y-20 lg:space-y-[6.25rem] lg:mt-[6.25rem] mt-20">
              <div className="">
                <h3 className="mb-5 xl:mb-8">Includes</h3>
                <ul className="space-y-8">
                  {room?.includes &&
                    room?.includes?.map((item, i) => (
                      <p
                        key={i}
                        className="flex text-balance items-center gap-[.625rem]"
                      >
                        <span>
                          <FaStarOfLife />
                        </span>{" "}
              ...
              {room?.additional &&
                room?.additional?.map((item, i) => (
                  <div key={i} className="">
                    <h3 className={`mb-5 xl:mb-8`}>{item?.title}</h3>
                    <p className={`text-justify`}>{item?.value}</p>
                  </div>
                ))}
              <div className="">
                <h3 className={`mb-5 xl:mb-8`}>Pets</h3>
                <p className={`text-justify`}>Pets are allowed.</p>
              </div>
              <div className="">
                <div className="sm:w-4/5 md:w-full">
                  {room?.packages && (
                    <h3 className="mb-4 text-left"> Available Packages: </h3>
                  )}
